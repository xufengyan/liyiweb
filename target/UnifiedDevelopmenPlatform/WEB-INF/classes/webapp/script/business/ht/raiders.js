 ...
 */
var jxmstc=new KBOperate("p_raiders");
var sst="";
...
function find(sst) {
    ...
    if(sst=="已审核"){
        findByPage(jxmstc.baseURL+"/findByPage","dgShipped",getCondition(sst),function () {
        ...
        findByPage(jxmstc.baseURL+"/findByPage", "dgRaiders",getCondition3(sst),function () {
            $("input[type=switchbutton]").switchbutton({onChange: function(checked){
                    var temp=$(this);
                    if (checked == true){//选中
                        var obj=new Object();
...
layui.use(['layer','element'],function () {
    ...
    jxmstc.browserForEasyui2(function (obj) {
        var toUrl=getRootPath()+"/p_raiders/uioperate?type=2&id="+obj.id;
        layer.open({
            title:'信息浏览',
            type: 2,
            area:["100%","100%"],
    ...
    jxmstc.editForEasyui2(function (obj) {
        var toUrl=getRootPath()+"/p_raiders/uioperate?type=1&id="+obj.id;
        layer.open({
            title:'信息修改',
            type: 2,
            area:["100%","100%"],
    ...
    jxmstc.delForEasyui("您确定删除当前选中的信息吗？",function (data) {
        if (data.code>0){
            layer.msg("删除成功！");
            jxmstc.findForEasyui("dgRaiders",getCondition());
        }else
        {
    ...
    $("#btnAdd").click(function () {
        var toUrl=getRootPath()+"/p_raiders/uioperate?type=0";
        layer.open({
            title:'信息新增',
            type: 2,
            area:["100%","100%"],
    ...
    $("#btnDeleteAll").click(function () {
        var ids=jxmstc.getEasyuiGridSelectRowsID("dgRaiders");
        if (ids=="") return;
