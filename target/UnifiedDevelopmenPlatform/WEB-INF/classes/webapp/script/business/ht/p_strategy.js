...
var jxmstc=new KBOperate("p_strategy");
...
function finddd()
{
      var obj = new Object();
      obj.condition = $("#txtName").val();
    findByPage(jxmstc.baseURL+"/findByPage", "dgstrategy", obj,function () {
...
function find() {
    ...
    jxmstc.findForEasyui("dgstrategy",getCondition());
   // finddd();
}
...
layui.use(['layer'],function () {
    ...
    jxmstc.browserForEasyuiph(function (obj) {
        var toUrl=getRootPath()+"/p_strategy/uioperate?type=2&id="+obj.id;
    ...
    jxmstc.editForEasyuiph(function (obj) {
        var toUrl=getRootPath()+"/p_strategy/uioperate?type=1&id="+obj.id;
        layer.open({
            title:'礼品信息修改',
            type: 2,
            area:["100%","100%"],
    ...
    $("#btnAdd").click(function () {
        var toUrl=getRootPath()+"/p_strategy/uioperate?type=0";
        layer.open({
            title:'联系店铺新增礼品',
            type: 2,
            area:["100%","100%"],
    ...
    $("#btnDeleteAll").click(function () {
        var ids=jxmstc.getEasyuiGridSelectRowsID("dgstrategy");
        if (ids=="") return;
