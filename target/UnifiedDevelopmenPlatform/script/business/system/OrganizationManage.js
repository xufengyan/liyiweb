...
function find() {
    ...
    kb.loadTreeGrid("findTree",con,"dgOrg",function () {
...
$(function(){
    ...
    $("#btnAdd").click(function () {
        var row=$("#dgOrg").treegrid("getSelected");//获取当前选中的行

        var toUrl=getRootPath()+"/admin/system/organization/organization.jsp";
        if (row!=null)
    ...
    $("i[type=edit]").live("click",function () {
        var toUrl=getRootPath()+"/admin/system/organization/organization.jsp?type=1&id="+$(this).attr("valueId");
        layer.open({
            title:'组织机构信息修改',
            type: 2,
            area:[600,350],
    ...
    $("i[type=del]").live("click",function () {
        var flag=confirm("只有当前组织机构没有子组织机构，而且当前组织机构没有被使用才可以删除，删除后数据不可以恢复，确定吗？");
        if (flag)
        {
            var obj=new Object();
