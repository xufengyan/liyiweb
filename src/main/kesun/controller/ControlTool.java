...
public class ControlTool {
	...
	/*根据页面传递的常规JSON参数，获取*/
	public static SearchViewParam GetViewParam(Map<String,Object> param)
	{
		SearchViewParam model=new SearchViewParam();
		try {
			model.pageNumber=Integer.parseInt(param.get("pageNumber").toString());
			model.rowsCount=Integer.parseInt(param.get("rowsCount").toString());
			return model;
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
