...
<body>
<div class="layui-table-tool" id="tbUnship">
    <div class="layui-inline">
        <label class="layui-form-label">订单编号:</label>
        <div class="layui-input-inline">
            <input type="text" class="layui-input" name="name" id="txtUnship" required  placeholder="请输入订单编号" autocomplete="off">
        </div>
    </div>
    <div class="layui-inline">
        <label class="layui-form-label">下单时间:</label>
        <div class="layui-input-inline">
            <input type="text" class="layui-input" id="test2" placeholder="请选择下单时间">
        </div>
    </div>
    <div class="layui-btn-group">
        <button class="layui-btn layui-btn-small" style="background-color: #008AE5;" id="bottnFind">
            <i class="layui-icon">&#xe615;</i>查询
        </button>
    </div>
    <button class="layui-btn layui-btn-small" style="background-color: #008AE5;" id="bottnDownLoad" >
        <i class="layui-icon">&#xe61e;</i>导出
    </button>
</div>
<table class="easyui-datagrid" title="已发货订单管理" style="height:auto" id="dgShipped"
       data-options="rownumbers:true,pagination:true,collapsible:true,striped:true,fit:true,toolbar:'#tbUnship'">
    <thead>
    <tr>
...
<!--导入数据导出-->
<form  style="display: none;"  id="frmDownloadExcelUnship" method="post" action="<%=basePath%>/business/loadout">
...
<script>
    ...
    $('#txtUnship').keyup(function () {
        // 1.获得搜索框的值；
        var keyWord=$('#txtUnship').val();
        search(keyWord);
    })


    $("#bottnFind").click(function () {
        find(statue);
    });
    ...
    layui.use(['layer','laydate','element'],function () {
        ...
        $("#bottnDownLoad").click(function () {
            $("#txtHNameUnship").val("");
            $("#frmDownloadExcelUnship").form().submit();
        });
